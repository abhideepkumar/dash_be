...
import { getAnswerContractPrompt, getRepairSQLPrompt } from '../prompts/contractPrompts.js';
...
export async function generateAnswerContract(query, tables) {
  ...
  const prompt = getAnswerContractPrompt({ query, schemaSummary });
...
export async function repairSQL(sql, contract, validationErrors, tables) {
  ...
  const prompt = getRepairSQLPrompt({ sql, contract, errorList, schemaContext });
